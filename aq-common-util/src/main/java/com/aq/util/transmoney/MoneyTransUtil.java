package com.aq.util.transmoney;

import com.aq.util.verify.VerifyUtil;
...
/**
 * 
 * @ClassName: MoneyTransUtil
 * @Description: 金额转换
 * @author: lijie
 * @date: 2018年1月30日 上午10:51:30
 */
public class MoneyTransUtil {
	...
	* @return
	...
	public static BigDecimal formatBigDecimal(BigDecimal bd) {
		if (null != bd) {
			return new BigDecimal(formativeFiveLength(bd));
		} else {
	...
	/**
	 * @author: 张霞
	 * @description：格式化保留五位小数
	 * @createDate: 15:38 2017/9/1
	 * @param bd
	 * @return: java.math.BigDecimal
	 * @version: 2.1
	 */
	public static BigDecimal formatByFive(BigDecimal bd) {
		if (null != bd) {
			DecimalFormat df = new DecimalFormat("#0.00000");
			return new BigDecimal(df.format(bd));
		} else {
	...
	public static String formatKeepTwo(String scoreStr) {
	...
	* @throws
	 */
	public static String strToBdScale(String money) {
	...
	public static String subMoneyScale(String stratMoney, String endMoney) {
		BigDecimal start = BigDecimal.ZERO;
		BigDecimal end = BigDecimal.ZERO;
		if (StringUtils.isNotBlank(stratMoney)) {
			start = new BigDecimal(stratMoney);
		}
		if (StringUtils.isNotBlank(endMoney)) {
			end = new BigDecimal(endMoney);
		}

		return start.subtract(end).setScale(2, BigDecimal.ROUND_HALF_UP).toString();
	}
}
