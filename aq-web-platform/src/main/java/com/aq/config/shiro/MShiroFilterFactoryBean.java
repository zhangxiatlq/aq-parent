...
public class MShiroFilterFactoryBean extends ShiroFilterFactoryBean {
    ...
    private Set<String> trustResource = new HashSet<>();
    ...
    private final class MSpringShiroFilter extends AbstractShiroFilter {
        ...
        protected void doFilterInternal(ServletRequest servletRequest, ServletResponse servletResponse,
                    ...
                    if ("JSESSIONID".equals(c.getName())) {
                        for (Session s : sessions) {
                            if (s.getId().equals(c.getValue())) {
                                //判断session是否失效
                                isHave = s.getLastAccessTime().getTime() + s.getTimeout() > System.currentTimeMillis();
                            }
                        }
                        if (!isHave) {
                            c.setValue(null);
            ...
            if (flag) {
                try {
                    //请求路径
                    log.info("请求路径：url={}", request.getRequestURL());
